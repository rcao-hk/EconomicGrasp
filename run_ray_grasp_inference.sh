...
: "${CKPT:?Set CKPT to Stage-1 or trained P2 checkpoint}"

GPUS="${GPUS:-0,1,2}"
SPLITS="${SPLITS:-test_seen,test_similar,test_novel}"
OUTPUT_ROOT="${OUTPUT_ROOT:-log/p2_ray_inference}"
...
case "${SCORE_MODE}" in auto|raw|supported) ;; *) echo "Invalid SCORE_MODE=${SCORE_MODE}" >&2; exit 2;; esac
case "${SELECTION}" in best|zero) ;; *) echo "Invalid SELECTION=${SELECTION}" >&2; exit 2;; esac
case "${RUN_EVAL}" in 0|1) ;; *) echo "RUN_EVAL must be 0 or 1" >&2; exit 2;; esac
if [[ "${RUN_EVAL}" == "1" && "${MAX_BATCHES}" != "0" ]]; then
  echo "RUN_EVAL=1 requires MAX_BATCHES=0; sampled AP needs a complete split." >&2
  exit 2
fi
...
launch() {
  ...
  mkdir -p "${out}"
  args=("${SCRIPT_DIR}/inference_ray_grasp.py"
    ...
    --ray_score_mode "${SCORE_MODE}"
    --ray_selection "${SELECTION}"
    --collision_thresh "${COLLISION_THRESH}"
    --collision_voxel_size "${COLLISION_VOXEL_SIZE}"
    --ray_max_batches "${MAX_BATCHES}")
  if [[ "${SAVE_NOCOLLISION}" == "1" ]]; then args+=(--save_nocollision); fi
  echo "[RAY][INFER] split=${split} gpu=${gpu} mode=${SCORE_MODE}/${SELECTION} out=${out}"
...
for raw in "${SPLIT_ARRAY[@]}"; do
  ...
  if [[ "${slot}" -ge "${#GPU_ARRAY[@]}" ]]; then
    ...
    PIDS=(); slot=0
    [[ "${failed}" == "0" ]] || { echo "Ray inference worker failed; inspect inference.log" >&2; exit 1; }
  fi
done
if [[ "${#PIDS[@]}" -gt 0 ]]; then
  failed=0
...
echo "[RAY] inference completed"

# Evaluate sequentially to avoid 3 simultaneous GraspNet multiprocessing pools.
if [[ "${RUN_EVAL}" == "1" ]]; then
  for raw in "${SPLIT_ARRAY[@]}"; do
    split="$(echo "${raw}" | xargs)"
    [[ -n "${split}" ]] || continue
    out="${OUTPUT_ROOT}/${split}"
    echo "[RAY][EVAL] split=${split} out=${out}"
    python "${SCRIPT_DIR}/inference_ray_grasp.py" \
      --dataset_root "${DATASET_ROOT}" \
      --camera "${CAMERA}" \
      --test_mode "${split}" \
      --save_dir "${out}" \
      --ray_eval_only \
      --ray_eval_workers "${EVAL_NUM_WORKERS}" \
      >"${out}/evaluation_stdout.log" 2>&1
    cat "${out}/ray_eval_summary.json"
  done
fi

echo "[RAY] completed: ${OUTPUT_ROOT}"
