...
class CenterViewAngleLegacyCandidateDecoder(nn.Module):
    ...
    def _run_branch_attention(self, tokens: torch.Tensor) -> torch.Tensor:
        """Run independent branch-token attention in bounded chunks."""
        if tokens.dim() != 3 or tokens.shape[1] not in (3, 4) or tokens.shape[2] != 64:
            raise ValueError(
                "legacy branch-attention tokens must be [N,3|4,64], got "
        ...
        for start in range(0, n, chunk):
            ...
            out, _ = self.branch_attn(part, part, part, need_weights=False)
            outputs.append(out)
...
class CenterViewAngleCDFCandidateDecoder(nn.Module):
...
# Backward-compatible direct import name used by earlier CDF scripts.
CenterViewAngleCandidateDecoder = CenterViewAngleCDFCandidateDecoder
...
class CenterViewAngleQueryTransformerLocalGraspModule(nn.Module):
    ...
    def __init__(
        ...
        if self.use_cdf:
            self.decoder = CenterViewAngleCDFCandidateDecoder(
                **decoder_common,
                num_cdf_thresholds=int(
                    self.config.num_cdf_thresholds
                ),
                cdf_increment_bias=float(
                    self.config.cdf_increment_bias
                ),
            )
        else:
            self.decoder = CenterViewAngleLegacyCandidateDecoder(
                **decoder_common
            )
    ...
    def forward(
        ...
        # 6) Mode-specific Center-View-Angle candidate decoder.
        end_points = self.decoder(group_features_a, end_points)

        # CDF diagnostics are meaningful only for the CDF head. Geometry
        # diagnostics remain independent and are handled by economicgrasp_dpt.
        if self.use_cdf:
            self._add_cdf_scalar_diagnostics(end_points)
            _, _, H, W = feat_map.shape
            self._save_cva_visualization(
                end_points,
                img=img,
                image_hw=(H, W),
            )
...
# =============================================================================
# Geometry-aware dense full-rotation proposal + sparse full-rotation CVA decoder
# =============================================================================


def _fullrot_matrix_to_6d(rot: torch.Tensor) -> torch.Tensor:
    """Continuous 6D rotation representation from the first two columns."""
    if rot.shape[-2:] != (3, 3):
        raise ValueError(f"rot must end with [3,3], got {tuple(rot.shape)}")
    return torch.cat([rot[..., :, 0], rot[..., :, 1]], dim=-1)


def _fullrot_score_expected(score_logits: torch.Tensor, class_dim: int = 1) -> torch.Tensor:
    """Expected quality for six-bin score logits."""
    if score_logits.shape[class_dim] != 6:
        raise ValueError(
            f"score logits must have 6 classes at dim={class_dim}, "
    ...
    shape = [1] * score_logits.dim()
    shape[class_dim] = 6
    return (F.softmax(score_logits, dim=class_dim) * bins.view(*shape)).sum(dim=class_dim)


class GeometryAwareDenseFieldRotNet(nn.Module):
    """Naive geometry-aware dense full-rotation proposal field.

    The module predicts a coarse field over ``view x in-plane angle`` anchors.
    It deliberately keeps the scorer simple and explicit:

        score = MLP([seed appearance/ray query,
                     full-rotation anchor token,
                     ray expressed in candidate rotation frame])

    It does not use surface normals, uncertainty, low-rank dot-product scoring,
    or robot-state conditioning.  Anchor chunks only control peak memory; they
    do not change the scorer formulation.

    Flat anchor convention:
        rotation_id = view_id * num_angle + angle_id
        angle = angle_id * pi / num_angle

    The in-plane angle offset is fixed to zero to match the extended-label
    convention.  It is intentionally not exposed as a constructor argument.

    Dense outputs:
        rotation_score:      [B,M,V,A]
        rotation_score_flat: [B,M,V*A]

    Sparse proposal outputs:
        rot_proposal_inds:       [B,M,L]
        rot_proposal_view_inds:  [B,M,L]
        rot_proposal_angle_inds: [B,M,L]
        rot_proposal_view_xyz:   [B,M,L,3]
        rot_proposal_rot:        [B,M,L,3,3]
        rot_proposal_score:      [B,M,L]
    """
    ...
    def __init__(
        ...
        hidden_dim: int = 128,
        num_rotation_queries: int = 1,
        view_dirs: Optional[torch.Tensor] = None,
        generate_grasp_views_fn: Optional[Callable[[int], torch.Tensor]] = None,
        batch_viewpoint_params_to_matrix_fn: Optional[
            Callable[[torch.Tensor, torch.Tensor], torch.Tensor]
        ] = None,
        rotation_chunk_size: int = 128,
        train_selection: str = "minmax_sample",
        replacement: bool = False,
        eps: float = 1e-6,
        vis_dir: Optional[str] = None,
        vis_every: int = 500,
        ...
        self.num_view = int(num_view)
        self.num_angle = int(num_angle)
        self.num_rotation = self.num_view * self.num_angle
        self.seed_feature_dim = int(seed_feature_dim)
        self.hidden_dim = int(hidden_dim)
        self.num_rotation_queries = int(num_rotation_queries)
        self.rotation_chunk_size = max(1, int(rotation_chunk_size))
        self.train_selection = str(train_selection)
        self.replacement = bool(replacement)
        self.eps = float(eps)
        self.vis_dir = vis_dir
        self.vis_every = max(1, int(vis_every))
        self.vis_num_seeds = max(1, int(vis_num_seeds))
        self._vis_iter = 0
        if self.vis_dir is not None:
            os.makedirs(self.vis_dir, exist_ok=True)

        if batch_viewpoint_params_to_matrix_fn is None:
            raise ValueError("batch_viewpoint_params_to_matrix_fn is required")
        self.batch_viewpoint_params_to_matrix_fn = batch_viewpoint_params_to_matrix_fn

        if view_dirs is None:
            if generate_grasp_views_fn is None:
                raise ValueError("Provide view_dirs or generate_grasp_views_fn")
            view_dirs = generate_grasp_views_fn(self.num_view)
        ...
        if view_dirs.shape != (self.num_view, 3):
            raise ValueError(
                f"view_dirs must be [{self.num_view},3], got {tuple(view_dirs.shape)}"
            )
        self.register_buffer("view_dirs", view_dirs)

        # Canonical full rotations, exactly matching the extended-label
        # convention: angles = [0, pi/A, ..., (A-1)pi/A], repeated for every
        # canonical approach view.  The flat order is view-major, angle-minor.
        angle_values = torch.tensor(
            [np.pi / float(self.num_angle) * i for i in range(self.num_angle)],
            dtype=view_dirs.dtype,
        )
        angles_repeat = angle_values.tile(self.num_view)
        views_repeat = view_dirs.repeat_interleave(self.num_angle, dim=0)
        full_rotations = self.batch_viewpoint_params_to_matrix_fn(
            -views_repeat,
            angles_repeat,
        ).view(self.num_view, self.num_angle, 3, 3)

        self.register_buffer("angle_values", angle_values)
        self.register_buffer("full_rotations", full_rotations)
        self.register_buffer(
            "flat_view_inds",
            torch.arange(self.num_view, dtype=torch.long).repeat_interleave(self.num_angle),
        )
        self.register_buffer(
            "flat_angle_inds",
            torch.arange(self.num_angle, dtype=torch.long).repeat(self.num_view),
        )

        # Appearance + camera-ray evidence only.
        self.app_mlp = nn.Sequential(
            nn.Linear(self.seed_feature_dim, self.hidden_dim),
            nn.ReLU(inplace=True),
        ...
        self.rotation_anchor_mlp = nn.Sequential(
        ...
        self.res_mlp = nn.Sequential(
            nn.Linear(self.hidden_dim, self.seed_feature_dim),
            nn.ReLU(inplace=True),
            nn.Linear(self.seed_feature_dim, self.seed_feature_dim),
        )

    @staticmethod
    def _ray_dirs_from_idx(
        idx_bm: torch.Tensor,
        K_b33: torch.Tensor,
        H: int,
        W: int,
    ...
    def _dense_rotation_score(
        self,
        q_seed: torch.Tensor,   # [B,M,H]
        ray_bm3: torch.Tensor,  # [B,M,3]
    ) -> torch.Tensor:
        B, M, H = q_seed.shape
        rot_flat = self.full_rotations.view(self.num_rotation, 3, 3)
        anchor_all = self.rotation_anchor_mlp(
            rot_flat.reshape(self.num_rotation, 9)
        )

        chunks = []
        for start in range(0, self.num_rotation, self.rotation_chunk_size):
            end = min(start + self.rotation_chunk_size, self.num_rotation)
            rot_chunk = rot_flat[start:end]       # [Rc,3,3]
            anchor_chunk = anchor_all[start:end]  # [Rc,H]
            Rc = end - start

            ray_local = torch.einsum(
                "bmc,rcd->bmrd",
                ray_bm3,
                rot_chunk,
            )  # [B,M,Rc,3]
            q_expand = q_seed.unsqueeze(2).expand(B, M, Rc, H)
            anchor_expand = anchor_chunk.view(1, 1, Rc, H).expand(B, M, Rc, H)
            score_in = torch.cat([q_expand, anchor_expand, ray_local], dim=-1)
            chunks.append(self.score_mlp(score_in).squeeze(-1))

        return torch.cat(chunks, dim=-1).contiguous()  # [B,M,V*A]

    def _make_sampling_prob(self, score_flat: torch.Tensor) -> torch.Tensor:
        score = torch.nan_to_num(
            score_flat.detach().float(),
            nan=0.0,
            posinf=0.0,
            neginf=0.0,
        )
        if self.train_selection == "softmax_sample":
            return F.softmax(score, dim=-1)
        if self.train_selection != "minmax_sample":
            raise ValueError(
                f"Unknown train_selection={self.train_selection}; "
                "use minmax_sample or softmax_sample"
            )
        smin = score.amin(dim=-1, keepdim=True)
        smax = score.amax(dim=-1, keepdim=True)
        weight = (score - smin) / (smax - smin).clamp_min(self.eps)
        denom = weight.sum(dim=-1, keepdim=True)
        prob = weight / denom.clamp_min(self.eps)
        uniform = torch.full_like(prob, 1.0 / float(self.num_rotation))
        return torch.where(denom > self.eps, prob, uniform)

    def _select_rotation_proposals(
        self,
        score_flat: torch.Tensor,  # [B,M,R]
        is_training: bool,
        forced_rotation_inds: Optional[torch.Tensor] = None,
    ) -> torch.Tensor:
        B, M, R = score_flat.shape
        L = min(max(1, self.num_rotation_queries), R)

        if forced_rotation_inds is not None:
            forced = forced_rotation_inds.detach().long().to(score_flat.device)
            if forced.dim() == 2:
                forced = forced.unsqueeze(-1)
            if forced.dim() != 3 or forced.shape[:2] != (B, M):
                raise ValueError(
                    "forced_rotation_inds must be [B,M] or [B,M,K], "
                    f"got {tuple(forced.shape)}"
                )
            forced = forced.clamp(0, R - 1)
            n_forced = min(forced.shape[-1], L)
            selected = forced[..., :n_forced]
            n_extra = L - n_forced
            if n_extra == 0:
                return selected.contiguous()

            prob = self._make_sampling_prob(score_flat).clone()
            prob.scatter_(dim=-1, index=selected, value=0.0)
            denom = prob.sum(dim=-1, keepdim=True)
            uniform = torch.full_like(prob, 1.0 / float(R))
            prob = torch.where(
                denom > self.eps,
                prob / denom.clamp_min(self.eps),
                uniform,
            )
            ...
            return torch.cat([selected, extra], dim=-1).contiguous()
        ...
        # Training proposals are sampled directly from the configured dense
        # rotation distribution.  There is no special predicted-top1 slot; for
        # L=1 this is exactly one multinomial sample per seed.
        prob = self._make_sampling_prob(score_flat)
        return torch.multinomial(
            prob.view(B * M, R),
            num_samples=L,
            replacement=self.replacement,
        ).view(B, M, L).contiguous()
    ...
    def _add_debug(
        ...
        proposal_inds: torch.Tensor,
    ) -> None:
        B, M, R = score_flat.shape
        L = proposal_inds.shape[-1]
        top2 = torch.topk(score_flat, k=min(2, R), dim=-1).values
        margin = top2[..., 0] - top2[..., -1]
        proposal_score = torch.gather(score_flat, -1, proposal_inds)
        end_points["D: RotNet R"] = score_flat.new_tensor(float(R)).reshape(())
        end_points["D: RotNet L"] = score_flat.new_tensor(float(L)).reshape(())
        end_points["D: RotNet score mean"] = score_flat.mean().reshape(())
        end_points["D: RotNet top1 score"] = top2[..., 0].mean().reshape(())
        end_points["D: RotNet top1 margin"] = margin.mean().reshape(())
        end_points["D: RotNet proposal score"] = proposal_score.mean().reshape(())
        end_points["D: RotNet proposal angle0"] = (
            (proposal_inds % self.num_angle) == 0
        ).float().mean().reshape(())


    @staticmethod
    def _make_bgr_canvas(img0: torch.Tensor, H: int, W: int) -> Optional[np.ndarray]:
        try:
            import cv2
            x = img0.detach().float().cpu()
            if x.dim() == 2:
    ...
    @staticmethod
    def _angle_bgr(angle_idx: int, num_angle: int) -> Tuple[int, int, int]:
        try:
            import cv2
            hue = int(round(179.0 * (int(angle_idx) % max(int(num_angle), 1)) / max(float(num_angle), 1.0)))
    ...
    def _maybe_visualize(
        ...
        """PNG-only RotNet diagnostics; no npy/npz artifacts are written."""
        ...
        B, M, R = score_flat.shape
        L = proposal_inds.shape[-1]
        score0 = score_flat[0].detach().float().cpu()
        proposal0 = proposal_inds[0].detach().long().cpu()
        prop_score0 = torch.gather(score0, 1, proposal0)
        top1 = score0.max(dim=-1).values
        top2 = torch.topk(score0, k=min(2, R), dim=-1).values
        margin0 = top2[:, 0] - top2[:, -1]

        # --------------------------------------------------------------
        # 1) Seed overlay: selected full-rotation angle and proposal score.
        # --------------------------------------------------------------
        img = end_points.get("img", None)
        canvas = self._make_bgr_canvas(img[0], H, W) if torch.is_tensor(img) else None
        if canvas is not None:
            try:
                import cv2
                idx0 = token_sel_idx[0].detach().long().cpu()
                # L is expected to be 1 in the current experiment; draw slot 0.
                rot_id = proposal0[:, 0]
                angle_id = rot_id % self.num_angle
                view_id = rot_id // self.num_angle
                ps = prop_score0[:, 0]
                ps_min = float(ps.min())
                ps_den = max(float(ps.max()) - ps_min, 1e-6)
                n = min(int(idx0.numel()), 1024)
                for j in range(n):
                    t = int(idx0[j])
                    u, v = int(t % W), int(t // W)
                    if not (0 <= u < W and 0 <= v < H):
                        continue
                    color = self._angle_bgr(int(angle_id[j]), self.num_angle)
                    strength = float((ps[j] - ps_min) / ps_den)
                    radius = 1 + int(round(3.0 * max(0.0, min(strength, 1.0))))
                    cv2.circle(canvas, (u, v), radius, color, thickness=-1, lineType=cv2.LINE_AA)
                text = (
                    f"RotNet it={self._vis_iter} L={L} "
                    f"mean={float(prop_score0.mean()):.3f} "
                    f"margin={float(margin0.mean()):.3f}"
                )
                cv2.putText(canvas, text, (8, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.48, (255,255,255), 2, cv2.LINE_AA)
                cv2.putText(canvas, text, (8, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.48, (0,0,0), 1, cv2.LINE_AA)
                cv2.imwrite(os.path.join(self.vis_dir, f"rotnet_proposal_overlay_it{self._vis_iter:06d}.png"), canvas)
            except Exception:
                pass

        # --------------------------------------------------------------
        # 2) Dense field heatmaps for a few high-score seeds.
        # --------------------------------------------------------------
        try:
            import matplotlib
            matplotlib.use("Agg")
            ...
            nseed = min(self.vis_num_seeds, M)
            seed_ids = torch.topk(top1, k=nseed, largest=True).indices
            pred_h = score0[seed_ids].view(nseed, self.num_view, self.num_angle).numpy()
            label = end_points.get("batch_grasp_rotation_score", None)
            has_label = torch.is_tensor(label) and tuple(label.shape[:4]) == (B, M, self.num_view, self.num_angle)
            lab_h = label[0, seed_ids.to(label.device)].detach().float().cpu().numpy() if has_label else None

            rows = nseed
            cols = 2 if has_label else 1
            fig, axes = plt.subplots(rows, cols, figsize=(4.8 * cols, 2.5 * rows), dpi=150, squeeze=False)
            for r in range(rows):
                im = axes[r, 0].imshow(pred_h[r], aspect="auto", interpolation="nearest")
                axes[r, 0].set_title(f"pred seed {int(seed_ids[r])}")
                axes[r, 0].set_ylabel("view")
                axes[r, 0].set_xlabel("angle")
                fig.colorbar(im, ax=axes[r, 0], fraction=0.025, pad=0.02)
                if has_label:
                    im2 = axes[r, 1].imshow(lab_h[r], aspect="auto", interpolation="nearest", vmin=0.0, vmax=1.0)
                    axes[r, 1].set_title(f"label seed {int(seed_ids[r])}")
                    axes[r, 1].set_ylabel("view")
                    axes[r, 1].set_xlabel("angle")
                    fig.colorbar(im2, ax=axes[r, 1], fraction=0.025, pad=0.02)
            fig.tight_layout()
            fig.savefig(os.path.join(self.vis_dir, f"rotnet_field_heatmap_it{self._vis_iter:06d}.png"))
            plt.close(fig)

            # ----------------------------------------------------------
            # 3) Proposal distributions.
            # ----------------------------------------------------------
            rot_id = proposal0.reshape(-1)
            angle_id = (rot_id % self.num_angle).numpy()
            view_id = (rot_id // self.num_angle).numpy()
            fig, axes = plt.subplots(2, 2, figsize=(11, 7), dpi=150)
            axes[0, 0].hist(angle_id, bins=np.arange(self.num_angle + 1) - 0.5)
            axes[0, 0].set_title("proposal angle histogram")
            axes[0, 0].set_xticks(np.arange(self.num_angle))
            axes[0, 1].hist(view_id, bins=min(30, self.num_view))
            axes[0, 1].set_title("proposal view histogram")
            axes[1, 0].hist(prop_score0.reshape(-1).numpy(), bins=40)
            axes[1, 0].set_title("proposal score")
            axes[1, 1].hist(margin0.numpy(), bins=40)
            axes[1, 1].set_title("dense top1-top2 margin")
            for a in axes.reshape(-1):
                a.grid(True, alpha=0.2)
            fig.tight_layout()
            fig.savefig(os.path.join(self.vis_dir, f"rotnet_proposal_hist_it{self._vis_iter:06d}.png"))
            plt.close(fig)
        except Exception:
            pass
    ...
    def forward(
        ...
        depth_map: torch.Tensor,      # [B,1,H,W], only H/W are used
        end_points: Dict[str, Any],
        ...
        if C != self.seed_feature_dim:
            raise ValueError(
                f"seed_features C={C}, expected {self.seed_feature_dim}"
            )
        if is_training is None:
            is_training = bool(self.training)

        ray_bm3 = self._ray_dirs_from_idx(token_sel_idx, K, H, W)
        seed_bmC = seed_features.transpose(1, 2).contiguous()
        app_tok = self.app_mlp(seed_bmC)
        ray_tok = self.ray_mlp(ray_bm3)
        q_seed = app_tok + ray_tok

        score_flat = self._dense_rotation_score(q_seed, ray_bm3)
        score = score_flat.view(B, M, self.num_view, self.num_angle)
        proposal_inds = self._select_rotation_proposals(
            score_flat,
            is_training=is_training,
            forced_rotation_inds=forced_rotation_inds,
        )

        flat_rot = self.full_rotations.view(self.num_rotation, 3, 3)
        flat_view_xyz = self.view_dirs.index_select(0, self.flat_view_inds)
        proposal_view_inds = self.flat_view_inds[proposal_inds]
        proposal_angle_inds = self.flat_angle_inds[proposal_inds]
        proposal_rot = flat_rot[proposal_inds]
        proposal_view_xyz = flat_view_xyz[proposal_inds]
        proposal_score = torch.gather(score_flat, dim=-1, index=proposal_inds)

        end_points["rotation_score"] = score
        end_points["rotation_score_flat"] = score_flat
        end_points["view_score"] = score.max(dim=-1).values
        end_points["rot_proposal_inds"] = proposal_inds
        end_points["rot_proposal_view_inds"] = proposal_view_inds
        end_points["rot_proposal_angle_inds"] = proposal_angle_inds
        end_points["rot_proposal_view_xyz"] = proposal_view_xyz
        end_points["rot_proposal_rot"] = proposal_rot
        end_points["rot_proposal_score"] = proposal_score

        res_feat = self.res_mlp(q_seed).transpose(1, 2).contiguous()
        self._add_debug(end_points, score_flat.detach(), proposal_inds.detach())
        self._maybe_visualize(
            end_points=end_points,
            token_sel_idx=token_sel_idx,
            score_flat=score_flat.detach(),
            proposal_inds=proposal_inds.detach(),
            H=H,
            W=W,
        )
        self._vis_iter += 1
...
class FullRotationCandidateDecoder(nn.Module):
    """D4RT-style final decoder for already specified full-rotation queries.

    Each concrete query is defined by:
        center + full rotation + coarse proposal score + grouped local evidence.

    Query-token processing mirrors ``CenterViewAngleCandidateDecoder``:
        1. fuse grouped feature, rotation embedding and proposal-score embedding;
        2. reshape to [B*M,L,C] and let the L full-rotation proposals of one
           center interact through lightweight transformer blocks;
        3. use EconomicGrasp-style depth/width/score branch interaction;
        4. apply dropout only immediately before the prediction heads.

    Inputs:
        grouped_features: [B,C,Q], Q=M*L
        full_rotation:    [B,Q,3,3]
        proposal_score:   [B,Q]
        end_points["fullrot_query_base_m"] = M
        end_points["fullrot_query_num_proposals"] = L
    ...
    def __init__(
        ...
        self.input_proj = nn.Conv1d(in_dim, self.hidden_dim, 1)
        self.rotation_embed = nn.Sequential(
            nn.Linear(6, self.hidden_dim),
            nn.ReLU(inplace=True),
            nn.Linear(self.hidden_dim, self.hidden_dim),
        )
        self.proposal_score_embed = nn.Sequential(
            nn.Linear(1, self.hidden_dim),
            nn.ReLU(inplace=True),
            nn.Linear(self.hidden_dim, self.hidden_dim),
        )

        # D4RT-style query-token interaction among L rotation proposals that
        # share the same center.  num_layers=0 cleanly disables this stage.
        self.layers = nn.ModuleList([
            SimpleAttentionModule(
                self.hidden_dim,
                n_head=max(1, int(num_heads)),
        ...
        # EconomicGrasp-style lightweight branch interaction.
        self.depth_proj = nn.Conv1d(self.hidden_dim, self.branch_dim, 1)
        self.width_proj = nn.Conv1d(self.hidden_dim, self.branch_dim, 1)
        self.score_proj = nn.Conv1d(self.hidden_dim, self.branch_dim, 1)
        self.branch_attn = nn.MultiheadAttention(
            self.branch_dim,
            num_heads=1,
            dropout=float(attn_dropout),
        ...
        self.depth_dropout = nn.Dropout(float(dropout_p))
        self.width_dropout = nn.Dropout(float(dropout_p))
        self.score_dropout = nn.Dropout(float(dropout_p))
        self.depth_head = nn.Conv1d(self.branch_dim, self.num_depth + 1, 1)
        self.width_head = nn.Conv1d(self.branch_dim, 1, 1)
        self.score_head = nn.Conv1d(self.branch_dim, 6, 1)
    ...
    def _add_debug(
        ...
        p = self.debug_prefix
        score_expected = _fullrot_score_expected(score_logits, class_dim=1)
        B, Q = score_expected.shape
        end_points[f"{p} final score"] = score_expected.mean().reshape(())
        end_points[f"{p} proposal score"] = proposal_score.mean().reshape(())

        valid_depth = depth_logits[:, :self.num_depth, :]
        depth_idx = valid_depth.argmax(dim=1)
        end_points[f"{p} depth01 ratio"] = (depth_idx <= 1).float().mean().reshape(())

        # Compare final scores among L proposals of each seed.
        if Q == int(base_m) * int(num_proposals):
            score_bml = score_expected.view(B, int(base_m), int(num_proposals))
            selected_rank = score_bml.argmax(dim=-1)
            end_points[f"{p} selected proposal rank0"] = (
                selected_rank == 0
            ).float().mean().reshape(())
            if int(num_proposals) > 1:
                top2 = torch.topk(score_bml, k=2, dim=-1).values
                end_points[f"{p} final score margin"] = (
                    top2[..., 0] - top2[..., 1]
                ).mean().reshape(())
            else:
                end_points[f"{p} final score margin"] = score_expected.new_tensor(0.0).reshape(())

        if "batch_rotation_query_pos_mask" in end_points:
            pos = end_points["batch_rotation_query_pos_mask"].bool()
            if pos.shape == score_expected.shape:
                end_points[f"{p} pos ratio"] = pos.float().mean().reshape(())
                if bool(pos.any()):
                    end_points[f"{p} score pos"] = score_expected[pos].mean().reshape(())
                if bool((~pos).any()):
                    end_points[f"{p} score neg"] = score_expected[~pos].mean().reshape(())

    def forward(
        self,
        grouped_features: torch.Tensor,  # [B,C,Q]
        full_rotation: torch.Tensor,     # [B,Q,3,3]
        proposal_score: torch.Tensor,    # [B,Q]
        end_points: Dict[str, Any],
    ) -> Dict[str, Any]:
        B, _, Q = grouped_features.shape
        base_m = int(end_points["fullrot_query_base_m"])
        num_proposals = int(end_points["fullrot_query_num_proposals"])
        if Q != base_m * num_proposals:
            raise RuntimeError(
                f"Expected Q=M*L={base_m}*{num_proposals}, got Q={Q}"
            )
        if full_rotation.shape != (B, Q, 3, 3):
            raise RuntimeError(
                f"full_rotation must be [B,Q,3,3], got {tuple(full_rotation.shape)}"
            )
        if proposal_score.shape != (B, Q):
            raise RuntimeError(
                f"proposal_score must be [B,Q], got {tuple(proposal_score.shape)}"
            )

        x = self.input_proj(grouped_features)
        rot_tok = self.rotation_embed(
            _fullrot_matrix_to_6d(full_rotation)
        ).permute(0, 2, 1).contiguous()
        proposal_tok = self.proposal_score_embed(
            proposal_score.unsqueeze(-1)
        ).permute(0, 2, 1).contiguous()
        x = x + rot_tok + proposal_tok

        # D4RT-style query interaction over the L candidate rotations of one seed.
        if len(self.layers) > 0 and num_proposals > 1:
            x = x.transpose(1, 2).contiguous().view(
                B,
                base_m,
                num_proposals,
            ...
            x = x.view(B * base_m, num_proposals, self.hidden_dim)
            for layer in self.layers:
                x = layer(x)
            x = x.view(B, base_m, num_proposals, self.hidden_dim)
            x = x.reshape(B, Q, self.hidden_dim).transpose(1, 2).contiguous()

        depth_feat = self.depth_proj(x)
        width_feat = self.width_proj(x)
        score_feat = self.score_proj(x)

        # Lightweight depth/width/score interaction for every concrete query.
        depth_tok = depth_feat.permute(0, 2, 1).reshape(B * Q, 1, self.branch_dim)
        width_tok = width_feat.permute(0, 2, 1).reshape(B * Q, 1, self.branch_dim)
        score_tok = score_feat.permute(0, 2, 1).reshape(B * Q, 1, self.branch_dim)
        branch_tokens = torch.cat([depth_tok, width_tok, score_tok], dim=1)
        branch_tokens, _ = self.branch_attn(
            branch_tokens,
            branch_tokens,
            branch_tokens,
            need_weights=False,
        ...
        depth_feat = branch_tokens[:, 0].view(B, Q, self.branch_dim).permute(0, 2, 1).contiguous()
        width_feat = branch_tokens[:, 1].view(B, Q, self.branch_dim).permute(0, 2, 1).contiguous()
        score_feat = branch_tokens[:, 2].view(B, Q, self.branch_dim).permute(0, 2, 1).contiguous()

        depth_logits = self.depth_head(self.depth_dropout(depth_feat))
        width_pred = self.width_head(self.width_dropout(width_feat))
        score_logits = self.score_head(self.score_dropout(score_feat))

        end_points["grasp_depth_pred"] = depth_logits
        end_points["grasp_width_pred"] = width_pred
        end_points["grasp_score_pred"] = score_logits
        end_points["grasp_score_expected"] = _fullrot_score_expected(
            score_logits,
            class_dim=1,
        )
...
class FullRotationCVALocalTransformer(nn.Module):
    """Sparse local decoder for full-rotation proposals.

    It flattens RotNet proposals [B,M,L] into Q=M*L concrete
    center-full-rotation query tokens, uses each full rotation to condition the
    local 2D/DPT grouping, and applies ``FullRotationCandidateDecoder``.
    """
    ...
    def __init__(
        ...
        self.head = FullRotationCandidateDecoder(
            num_depth=self.num_depth,
            in_dim=int(self.config.head_model_dim),
            hidden_dim=int(head_hidden_dim),
            num_layers=int(head_num_layers),
            num_heads=int(head_num_heads),
            branch_dim=int(head_branch_dim),
            dropout_p=float(head_dropout_p),
            attn_dropout=float(head_attn_dropout),
            debug_prefix="D: FullRot",
        )
    ...
    def _flatten_proposals(
        ...
        token_sel_idx: torch.Tensor,       # [B,M]
        proposal_view_inds: torch.Tensor,  # [B,M,L]
        proposal_angle_inds: torch.Tensor, # [B,M,L]
        proposal_view_xyz: torch.Tensor,   # [B,M,L,3]
        proposal_rot: torch.Tensor,        # [B,M,L,3,3]
        proposal_score: torch.Tensor,      # [B,M,L]
    ):
        B, C, M = seed_features.shape
        L = proposal_view_inds.shape[-1]
        Q = M * L
        feat_q = (
            seed_features.transpose(1, 2)
            .unsqueeze(2)
            .expand(B, M, L, C)
            .reshape(B, Q, C)
            .transpose(1, 2)
        ...
        xyz_q = seed_xyz.unsqueeze(2).expand(B, M, L, 3).reshape(B, Q, 3).contiguous()
        token_q = token_sel_idx.unsqueeze(-1).expand(B, M, L).reshape(B, Q).contiguous()
        view_q = proposal_view_inds.reshape(B, Q).contiguous()
        angle_q = proposal_angle_inds.reshape(B, Q).contiguous()
        view_xyz_q = proposal_view_xyz.reshape(B, Q, 3).contiguous()
        rot_q = proposal_rot.reshape(B, Q, 3, 3).contiguous()
        proposal_score_q = proposal_score.reshape(B, Q).contiguous()
        return (
            feat_q,
            xyz_q,
            token_q,
    ...
    @staticmethod
    def _call_label_process(
        process_grasp_labels_fn: Callable[..., Any],
        end_points: Dict[str, Any],
        kwargs: Optional[Dict[str, Any]],
    ) -> Dict[str, Any]:
        out = process_grasp_labels_fn(end_points, **(kwargs or {}))
        if isinstance(out, tuple):
            return out[-1]
        if isinstance(out, dict):
            return out
        raise TypeError(
            "process_grasp_labels_fn must return dict or tuple ending in dict"
        )

    @staticmethod
    def _gather_selected_angle_labels(
        end_points: Dict[str, Any],
    ) -> Dict[str, Any]:
        """Gather extended [B,Q,A] labels at each proposal's fixed angle."""
        angle = end_points["grasp_top_angle_inds"].long()
        B, Q = angle.shape

        def gather_value(key: str) -> torch.Tensor:
            x = end_points[key]
            if x.shape[:2] != (B, Q):
                raise RuntimeError(
                    f"{key} first dims {tuple(x.shape[:2])}, expected {(B, Q)}"
                )
            if x.dim() != 3:
                raise RuntimeError(
                    f"{key} must be [B,Q,A], got {tuple(x.shape)}"
                )
            idx = angle.clamp(0, x.shape[-1] - 1).unsqueeze(-1)
            return torch.gather(x, dim=-1, index=idx).squeeze(-1)

        score = gather_value("batch_grasp_score_angle")
        depth = gather_value("batch_grasp_depth_angle").long()
        width = gather_value("batch_grasp_width_angle")
        collision = gather_value("batch_grasp_collision_angle")
        valid = gather_value("batch_grasp_angle_valid_mask").bool()
        pos = gather_value("batch_grasp_angle_pos_mask").bool()

        end_points["batch_grasp_rotations"] = angle
        end_points["batch_grasp_score"] = score
        end_points["batch_grasp_depth"] = depth
        end_points["batch_grasp_width"] = width
        end_points["batch_grasp_collision"] = collision
        end_points["batch_valid_mask"] = valid
        end_points["batch_rotation_query_valid_mask"] = valid
        end_points["batch_rotation_query_pos_mask"] = pos
        return end_points

    @torch.no_grad()
    def _add_debug(self, end_points: Dict[str, Any]) -> None:
        angle = end_points["grasp_top_angle_inds"]
        score = end_points["grasp_score_expected"]
        end_points["D: FullRot Q"] = score.new_tensor(float(score.shape[1])).reshape(())
        end_points["D: FullRot selected angle0"] = (
            angle == 0
        ).float().mean().reshape(())
        end_points["D: FullRot final score"] = score.mean().reshape(())


    @staticmethod
    def _make_local_canvas(img0: torch.Tensor, H: int, W: int) -> Optional[np.ndarray]:
        try:
            import cv2
            x = img0.detach().float().cpu()
            if x.dim() == 2:
            ...
            x = x - x.amin()
            x = x / (x.amax() + 1e-6)
            canvas = (x.permute(1, 2, 0).numpy() * 255.0).astype(np.uint8)[..., ::-1].copy()
            if canvas.shape[:2] != (H, W):
                canvas = cv2.resize(canvas, (W, H), interpolation=cv2.INTER_LINEAR)
            return canvas
        except Exception:
    ...
    @staticmethod
    def _local_angle_bgr(angle_idx: int, num_angle: int) -> Tuple[int, int, int]:
        try:
            import cv2
            hue = int(round(179.0 * (int(angle_idx) % max(int(num_angle), 1)) / max(float(num_angle), 1.0)))
            hsv = np.uint8([[[hue, 230, 255]]])
    ...
    def _maybe_visualize(
        ...
        """PNG-only local full-rotation diagnostics; no npy/npz writes."""
        cfg = self.config
        if cfg.vis_dir is None:
            return
        if self._vis_iter % max(1, int(cfg.vis_every)) != 0:
            return
        if not _rank0_only():
            return
        os.makedirs(cfg.vis_dir, exist_ok=True)

        score = end_points.get("grasp_score_expected", None)
        angle = end_points.get("grasp_top_angle_inds", None)
        token = end_points.get("token_sel_idx", None)
        proposal = end_points.get("grasp_rotation_proposal_score", None)
        if not (torch.is_tensor(score) and torch.is_tensor(angle) and torch.is_tensor(token)):
            return

        score0 = score[0].detach().float().cpu()
        angle0 = angle[0].detach().long().cpu()
        token0 = token[0].detach().long().cpu()
        proposal0 = proposal[0].detach().float().cpu() if torch.is_tensor(proposal) else torch.zeros_like(score0)
        Q = int(score0.numel())
        valid = end_points.get("batch_rotation_query_valid_mask", None)
        pos = end_points.get("batch_rotation_query_pos_mask", None)
        valid0 = valid[0].detach().bool().cpu() if torch.is_tensor(valid) and valid.shape == score.shape else torch.ones(Q, dtype=torch.bool)
        pos0 = pos[0].detach().bool().cpu() if torch.is_tensor(pos) and pos.shape == score.shape else None

        # --------------------------------------------------------------
        # 1) Final-query overlay + local grouping preview.
        # --------------------------------------------------------------
        img = end_points.get("img", None)
        canvas = self._make_local_canvas(img[0], H, W) if torch.is_tensor(img) else None
        if canvas is not None:
            try:
                import cv2
                q_pool = torch.where(valid0)[0]
                if q_pool.numel() == 0:
                    q_pool = torch.arange(Q)
                nvis = min(int(getattr(cfg, "vis_num_queries", 128)), int(q_pool.numel()))
                top_local = torch.topk(score0[q_pool], k=nvis, largest=True).indices
                q_sel = q_pool[top_local]
                smin = float(score0[q_sel].min()) if q_sel.numel() else 0.0
                sden = max(float(score0[q_sel].max()) - smin, 1e-6) if q_sel.numel() else 1.0
                for q in q_sel.tolist():
                    t = int(token0[q])
                    u, v = int(t % W), int(t // W)
                    if not (0 <= u < W and 0 <= v < H):
                        continue
                    color = self._local_angle_bgr(int(angle0[q]), self.num_angle)
                    strength = (float(score0[q]) - smin) / sden
                    radius = 1 + int(round(4.0 * max(0.0, min(strength, 1.0))))
                    cv2.circle(canvas, (u, v), radius, color, thickness=-1, lineType=cv2.LINE_AA)
                    if pos0 is not None and not bool(pos0[q]):
                        cv2.circle(canvas, (u, v), radius + 2, (0, 0, 255), thickness=1, lineType=cv2.LINE_AA)

                # Draw the grouping preview captured for the first queries.
                puv = end_points.get("kview_debug_patch_uv0", None)
                pattn = end_points.get("kview_debug_patch_attn0", None)
                cuv = end_points.get("kview_debug_center_uv0", None)
                vy = end_points.get("kview_debug_vec_y0", None)
                vz = end_points.get("kview_debug_vec_z0", None)
                if torch.is_tensor(puv):
                    puv = puv.detach().float().cpu().numpy()
                    pattn_np = pattn.detach().float().cpu().numpy() if torch.is_tensor(pattn) else None
                    cuv_np = cuv.detach().float().cpu().numpy() if torch.is_tensor(cuv) else None
                    vy_np = vy.detach().float().cpu().numpy() if torch.is_tensor(vy) else None
                    vz_np = vz.detach().float().cpu().numpy() if torch.is_tensor(vz) else None
                    n_patch = min(puv.shape[0], int(getattr(cfg, "vis_num_patch_queries", 8)))
                    for q in range(n_patch):
                        pts = puv[q].reshape(-1, 2)
                        aw = pattn_np[q].reshape(-1) if pattn_np is not None and q < pattn_np.shape[0] else np.ones(len(pts), dtype=np.float32)
                        aw = (aw - aw.min()) / max(float(aw.max() - aw.min()), 1e-6)
                        color = self._local_angle_bgr(int(angle0[q]), self.num_angle)
                        for (uu, vv), a in zip(pts, aw):
                            u, v = int(round(float(uu))), int(round(float(vv)))
                            if 0 <= u < W and 0 <= v < H:
                                alpha = 0.25 + 0.75 * float(a)
                                c = tuple(int(alpha * cc) for cc in color)
                                cv2.circle(canvas, (u, v), 1 + int(a > 0.65), c, thickness=-1, lineType=cv2.LINE_AA)
                        if cuv_np is not None and q < len(cuv_np):
                            c0 = (int(round(cuv_np[q,0])), int(round(cuv_np[q,1])))
                            if 0 <= c0[0] < W and 0 <= c0[1] < H:
                                cv2.circle(canvas, c0, 3, (255,255,255), thickness=-1)
                                if vy_np is not None and q < len(vy_np):
                                    p1 = (int(round(c0[0] + vy_np[q,0])), int(round(c0[1] + vy_np[q,1])))
                                    cv2.line(canvas, c0, p1, (0,255,255), 1, cv2.LINE_AA)
                                if vz_np is not None and q < len(vz_np):
                                    p2 = (int(round(c0[0] + vz_np[q,0])), int(round(c0[1] + vz_np[q,1])))
                                    cv2.line(canvas, c0, p2, (0,255,0), 1, cv2.LINE_AA)

                txt = f"FullRot it={self._vis_iter} Q={Q} final={float(score0.mean()):.3f} coarse={float(proposal0.mean()):.3f}"
                cv2.putText(canvas, txt, (8,20), cv2.FONT_HERSHEY_SIMPLEX, 0.48, (255,255,255), 2, cv2.LINE_AA)
                cv2.putText(canvas, txt, (8,20), cv2.FONT_HERSHEY_SIMPLEX, 0.48, (0,0,0), 1, cv2.LINE_AA)
                cv2.imwrite(os.path.join(cfg.vis_dir, f"fullrot_local_overlay_it{self._vis_iter:06d}.png"), canvas)
            except Exception:
                pass

        # --------------------------------------------------------------
        # 2) Coarse proposal score versus final local score.
        # --------------------------------------------------------------
        try:
            import matplotlib
            matplotlib.use("Agg")
            ...
            if pos0 is not None:
                ...
                pp = valid0 & pos0
                if bool(neg0.any()):
                    axes[0].scatter(proposal0[neg0].numpy(), score0[neg0].numpy(), s=5, alpha=0.35, label="negative")
                    axes[1].hist(score0[neg0].numpy(), bins=35, alpha=0.55, label="negative")
                if bool(pp.any()):
                    axes[0].scatter(proposal0[pp].numpy(), score0[pp].numpy(), s=7, alpha=0.55, label="positive")
                    axes[1].hist(score0[pp].numpy(), bins=35, alpha=0.55, label="positive")
            else:
                axes[0].scatter(proposal0.numpy(), score0.numpy(), s=5, alpha=0.4)
                axes[1].hist(score0.numpy(), bins=35)
            axes[0].set_xlabel("RotNet proposal score")
            axes[0].set_ylabel("final expected score")
            axes[0].set_title("coarse vs local score")
            ...
            for a in axes:
                ...
                if handles:
                    a.legend()
            fig.tight_layout()
            fig.savefig(os.path.join(cfg.vis_dir, f"fullrot_score_diagnostics_it{self._vis_iter:06d}.png"))
            plt.close(fig)

            # ----------------------------------------------------------
            # 3) Angle and depth distributions.
            # ----------------------------------------------------------
            depth_logits = end_points.get("grasp_depth_pred", None)
            fig, axes = plt.subplots(1, 2, figsize=(10,4), dpi=150)
            axes[0].hist(angle0[valid0].numpy(), bins=np.arange(self.num_angle + 1)-0.5)
            axes[0].set_xticks(np.arange(self.num_angle))
            axes[0].set_title("proposal angle distribution")
            if torch.is_tensor(depth_logits):
                dp = depth_logits[0, :self.num_depth].detach().float().cpu().argmax(dim=0)
                axes[1].hist(dp[valid0].numpy(), bins=np.arange(self.num_depth + 1)-0.5, alpha=0.65, label="pred")
                label_depth = end_points.get("batch_grasp_depth", None)
                if torch.is_tensor(label_depth) and pos0 is not None and bool(pos0.any()):
                    ld = label_depth[0].detach().long().cpu()
                    axes[1].hist(ld[pos0].numpy(), bins=np.arange(self.num_depth + 1)-0.5, alpha=0.55, label="label-pos")
                axes[1].set_xticks(np.arange(self.num_depth))
                axes[1].set_title("depth distribution")
                axes[1].legend()
            for a in axes:
                a.grid(True, alpha=0.2)
            fig.tight_layout()
            fig.savefig(os.path.join(cfg.vis_dir, f"fullrot_pose_hist_it{self._vis_iter:06d}.png"))
            plt.close(fig)
        except Exception:
            pass
    ...
    def forward(
        ...
        required = [
            "rot_proposal_view_inds",
            "rot_proposal_angle_inds",
            "rot_proposal_view_xyz",
            "rot_proposal_rot",
            "rot_proposal_score",
        ]
        missing = [k for k in required if k not in end_points]
        if missing:
            raise KeyError(f"Missing RotNet proposal keys: {missing}")
        ...
        (
            ...
            base_m,
            num_proposals,
        ) = self._flatten_proposals(
            seed_features,
            seed_xyz,
            token_sel_idx,
            end_points["rot_proposal_view_inds"],
            end_points["rot_proposal_angle_inds"],
            end_points["rot_proposal_view_xyz"],
            end_points["rot_proposal_rot"],
            end_points["rot_proposal_score"],
        )

        end_points["fullrot_query_base_m"] = int(base_m)
        end_points["fullrot_query_num_proposals"] = int(num_proposals)
        end_points["xyz_graspable"] = xyz_q
        end_points["token_sel_idx"] = token_q
        end_points["token_sel_xyz"] = xyz_q
        end_points["grasp_top_view_inds"] = view_q
        end_points["grasp_top_angle_inds"] = angle_q
        end_points["grasp_top_view_xyz"] = view_xyz_q
        end_points["grasp_top_view_rot"] = rot_q
        end_points["grasp_top_rotation"] = rot_q
        end_points["grasp_rotation_proposal_score"] = proposal_score_q

        # Label availability and proposal-selection mode are intentionally
        # decoupled. During validation model.eval() makes is_training=False so
        # RotNet uses deterministic top-L proposals, while process_fn remains
        # non-None and still generates GT labels for validation loss/metrics.
        if process_grasp_labels_fn is not None:
            end_points = self._call_label_process(
                process_grasp_labels_fn,
                end_points,
                process_grasp_labels_kwargs,
            )
            end_points = self._gather_selected_angle_labels(end_points)

        grouped = self.group(
            seed_features=feat_q,
            token_sel_idx=token_q,
            seed_xyz=xyz_q,
            top_view_rot=rot_q,
            feat_map=feat_map,
            depth_map=depth_map,
        ...
        end_points = self.head(
            grouped,
            full_rotation=rot_q,
            proposal_score=proposal_score_q,
            end_points=end_points,
        )

        # Fixed-angle compatibility logits.  The proposal already specifies the
        # full rotation, so these are not trained by an angle CE.
        B, Q = angle_q.shape
        angle_logits = grouped.new_full((B, self.num_angle + 1, Q), -20.0)
        angle_logits.scatter_(
            1,
            angle_q.unsqueeze(1).clamp(0, self.num_angle - 1),
            0.0,
        )
        end_points["grasp_angle_pred"] = angle_logits

        self._add_debug(end_points)
        _, _, H, W = feat_map.shape
        self._maybe_visualize(end_points, H=H, W=W)
        self._vis_iter += 1
        return end_points


class GeometryAwareRotProposalCVALocalGraspModule(nn.Module):
    """Convenience wrapper owning RotNet and the sparse local decoder."""

    def __init__(
        self,
        rotation_net: GeometryAwareDenseFieldRotNet,
        local_transformer: FullRotationCVALocalTransformer,
    ):
        super().__init__()
    ...
    def forward(
        ...
        end_points, res_feat = self.rotation_net(
            seed_features=seed_features,
            token_sel_idx=token_sel_idx,
            K=camera_K,
            depth_map=depth_map,
        ...
        seed_features = seed_features + res_feat
        return self.local_transformer(
            ...
            end_points=end_points,
            is_training=is_training,
            process_grasp_labels_fn=process_grasp_labels_fn,
            process_grasp_labels_kwargs=process_grasp_labels_kwargs,
            objectness_logits=objectness_logits,
            graspness_map=graspness_map,
        )
