...
def get_loss_legacy(end_points):
    """Explicit-angle legacy CVA objective from the extended cache."""
    depth_reg_loss, end_points = compute_depth_reg_loss(end_points)
    objectness_loss, end_points = compute_objectness_loss_tok(end_points)
    graspness_loss, end_points = compute_graspness_loss_tok(end_points)
    view_loss, end_points = compute_view_graspness_loss(end_points)
...
def get_loss(end_points, use_cdf: bool = False):
    """Dispatch to the loss matching the active CVA prediction head."""
    if bool(use_cdf):
        return get_loss_cdf(end_points)
    return get_loss_legacy(end_points)
