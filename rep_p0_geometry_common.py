...
REP_P0_CACHE_CONTRACT_VERSION = 2
...
def scene_sharded_indices(
    total: int,
    interval: float,
    shard_id: int,
    num_shards: int,
    *,
    frames_per_scene: int = 256,
    max_samples: int = 0,
) -> list[int]:
    """Select frames using scene-level rather than frame-level sharding.

    Each local scene position is owned by exactly one shard:
        scene_position % num_shards == shard_id

    This prevents every miner from touching every scene and therefore avoids
    replicating the exact evaluator scene CAD/DexNet cache across all GPUs.
    """
    total = int(total)
    ...
    out: list[int] = []
    for scene_pos, start in enumerate(range(0, total, frames_per_scene)):
        if scene_pos % num_shards != shard_id:
            continue
        end = min(start + frames_per_scene, total)
        out.extend(range(start, end, stride))
        if max_samples > 0 and len(out) >= int(max_samples):
            return out[: int(max_samples)]
    return out
...
def prepare_cad_models_for_evidence(
    evaluator,
    scene_id: int,
    ...
    raw_models = pack[0]
    models = [
        voxel_downsample_numpy(np.asarray(model, dtype=np.float32), voxel_size)
...
def build_evidence_table(
    ...
    """Build the Rep-P0 table once at the evidence resolution."""
...
def load_full_cad_scene_cloud(
    evaluator,
    scene_id: int,
    anno_id: int,
    voxel_size: float = REP_P0_EVIDENCE_VOXEL_SIZE,
    *,
    prepared_models: Sequence[np.ndarray] | None = None,
    table_points: np.ndarray | None = None,
) -> np.ndarray:
    """Return complete CAD objects + table at the Rep-P0 evidence resolution.

    prepared_models should normally be a scene-local 5-mm object-frame cache.
    Passing it prevents raw CAD/Dex-Net scene data from being reloaded on every
    frame. Exact-action labels remain generated by the untouched evaluator.
    """
    from graspnetAPI.utils.eval_utils import transform_points

    if prepared_models is None:
        prepared_models = prepare_cad_models_for_evidence(
            evaluator, scene_id, voxel_size
        )
    if table_points is None:
        table_points = build_evidence_table(voxel_size)
    ...
    models_cam = [
        ...
        for obj_index, model in enumerate(prepared_models)
    ]
    table_cam = transform_points(
        np.asarray(table_points, dtype=np.float32),
