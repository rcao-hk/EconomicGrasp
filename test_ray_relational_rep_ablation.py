#!/usr/bin/env python3
"""Held-out exact-action test for fully trainable relational representation ablations.

Every checkpoint contains a complete trained selector. Only its input
representation differs. Stage-1 remains frozen and the exact-action evaluator is
used only after the selector chooses a physical center.
"""
...
def uniform_pick(indices, count):
    ...
    if len(pos) < count:
        used = torch.zeros(len(indices), dtype=torch.bool, device=indices.device)
        used[pos] = True
...
def evaluate_grid(evaluator, scene_id, anno_id, grasps, valid):
    ...
    friction[ids] = r.friction
    assigned[ids] = r.assigned_obj
    coll[ids] = r.collision_or_empty.astype(np.int8)
    pure[ids] = r.pure_collision.astype(np.int8)
    empty[ids] = r.empty.astype(np.int8)
...
def main():
    ...
    if ckpt.get("selector_type") != "cross_center_relational_rep_ablation_v1":
        raise RuntimeError(f"Unexpected selector_type={ckpt.get('selector_type')!r}")
    mode = str(ckpt["representation_mode"])
    if mode not in REPRESENTATION_MODES:
        raise RuntimeError(f"Unknown checkpoint representation_mode={mode!r}")
    ...
    move_threshold = float(
        ckpt.get("move_threshold", 0.5)
        if ARGS.move_threshold is None
    ...
    load_stage1(model, ARGS.checkpoint_path)
    model.eval()
    ...
    if ARGS.save_candidate_rows:
        cand_file = gzip.open(out_dir / "per_candidate.csv.gz", "wt", newline="")
        fields = [
            "split","scene_id","anno_id","query_id","offset_mm","valid",
            "raw_score","selector_logit","delta_pred","utility","friction",
            "selected_learned","selected_raw","selected_oracle",
        ]
        cand_writer = csv.DictWriter(cand_file, fieldnames=fields)
    ...
    try:
        for local_i, batch in enumerate(loader):
            ...
            with torch.inference_mode():
                for k in range(len(offsets)):
                    epk, grouped = rerun_cdf_with_read_center(
                        model,
                        ep,
                        read_center=centers[k],
                        output_center=centers[k],
                    )
                    ...
                    pred = (
                        native_pred
                        if k == zero_k
                        else pred_decode_center_view_angle(epk, use_cdf=True)[0]
                    )
            ...
            sel = torch.stack(sel_k)
            mean = torch.stack(mean_k)
            ...
            valid_kn_t = torch.from_numpy(valid).to(device)
            valid_nk_t = valid_kn_t.transpose(0, 1).contiguous()
            ...
            if tokens.shape[-1] != selector.token_dim:
                raise RuntimeError(
                    f"Token mismatch for {mode}: {tokens.shape[-1]} vs {selector.token_dim}"
                )

            with torch.inference_mode():
                rel = selector(
                    (tokens-token_mean) / token_std.clamp_min(1e-5),
                    valid_nk_t,
                    zero_k,
                )
            ...
            scene_id = int(batch["scene_idx"].reshape(-1)[0])
            anno_id = int(batch["anno_idx"].reshape(-1)[0])
            ...
            for j, qid in enumerate(qids):
                native_u = float(mets["native"]["utility"][j])
                learned_u = float(mets["learned"]["utility"][j])
                row = {
                    ...
                    "move_probability": float(move_prob[j]),
                ...
                for name in ("native","raw","learned","oracle"):
                    m = mets[name]
                    for key in (
                        "utility","success04","success08","collision_or_empty",
                        "pure_collision","empty","assigned_obj",
                    ):
                        row[f"{name}_{key}"] = (
                            float(m[key][j]) if key == "utility" else int(m[key][j])
                        )

                row["learned_rescue08"] = int(
                    (not row["native_success08"]) and row["learned_success08"]
                )
                row["learned_harm08"] = int(
                    row["native_success08"] and (not row["learned_success08"])
                )
                row["raw_rescue08"] = int(
                    (not row["native_success08"]) and row["raw_success08"]
                )
                row["raw_harm08"] = int(
                    row["native_success08"] and (not row["raw_success08"])
                )
                ...
                if cand_writer:
                    for k, o in enumerate(offsets):
                        cand_writer.writerow({
                            ...
                            "offset_mm": float(o),
                            "valid": int(valid[k, j]),
                            "raw_score": float(raw_score[k, j]),
                            "selector_logit": float(selector_logits[j, k]),
                            "delta_pred": float(delta_pred[j, k]),
                            "utility": float(utility[k, j]),
                            "friction": float(mats["friction"][k, j]),
                            "selected_learned": int(k == learned_k[j]),
                            "selected_raw": int(k == raw_k[j]),
            ...
            sample = {
                ...
                "native_forward_sec": native_sec,
                "reread_sec": reread_sec,
                "exact_eval_sec": et["eval_sec"],
                "move_rate": float((learned_k != zero_k).mean()),
                "move_probability_mean": float(move_prob.mean()),
            }
            for name in mets:
            ...
            timing["native_forward_sec"] += native_sec
            timing["reread_sec"] += reread_sec
            for key, val in et.items():
                timing[key] += float(val)

            if local_i % 20 == 0:
                print(
                    f"[REP-ABL-TEST][{mode}] {local_i+1}/{len(indices)} "
                    f"scene={scene_id:04d} anno={anno_id:04d} "
                    f"native08={sample['native_success08']:.3f} "
                    f"learned08={sample['learned_success08']:.3f} "
                    f"oracle08={sample['oracle_success08']:.3f} "
                    f"move={sample['move_rate']:.3f}",
                    flush=True,
                )
    finally:
        if cand_file:
    ...
    write_csv(out_dir / "per_query.csv", rows)
    write_csv(out_dir / "per_sample_summary.csv", sample_rows)

    def agg(name):
        return {
            metric: float(np.mean([r[f"{name}_{metric}"] for r in rows]))
            for metric in (
                "utility","success04","success08","collision_or_empty",
                "pure_collision","empty",
            )
        }

    summary = {
        "protocol": "fully trainable relational representation ablation v1",
        "representation_mode": mode,
        "split": ARGS.split,
    ...
    summary["gaps"] = {
        "learned_minus_native_utility": (
            summary["aggregate"]["learned"]["utility"]
            - summary["aggregate"]["native"]["utility"]
        ),
        "learned_minus_native_success08": (
            summary["aggregate"]["learned"]["success08"]
            - summary["aggregate"]["native"]["success08"]
        ),
        "oracle_minus_learned_utility": (
            summary["aggregate"]["oracle"]["utility"]
            - summary["aggregate"]["learned"]["utility"]
        ),
        "oracle_minus_learned_success08": (
            summary["aggregate"]["oracle"]["success08"]
            - summary["aggregate"]["learned"]["success08"]
        ),
    }
    headroom = (
    ...
    summary["utility_headroom_recovery"] = (
        summary["gaps"]["learned_minus_native_utility"] / headroom
