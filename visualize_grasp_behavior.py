...
def case_visuals(writer, out, snapshot, rgb, raw_item, K,
                 items, topk, local_queries, point_stride,
                 e1_model=None, air_model=None):
    images: List[Tuple[str, Path]] = []
    files: List[Tuple[str, Path]] = []

    if "rgb" in items:
        p = out / "rgb.png"
        save_rgb(p, rgb, "RGB input (cropped/resized model view)")
        images.append(("RGB", p))
    ...
    if "depth" in items or "pointcloud" in items:
        ...
        for name in ("pointcloud_pred_nominal.ply", "pointcloud_active.ply",
    ...
    if "proposal" in items:
        save_proposal_bundle(out, rgb, snapshot["proposal_logits"])
        for name in ("objectness_overlay.png", "graspness_overlay.png"):
            images.append((name, out / name))

    if "feature" in items:
        save_feature_bundle(out, snapshot["feature_pre"], snapshot["feature_post"])
        images.extend([
            ("Pre-enhancer PCA", out / "feature_pre_enhancer_pca.png"),
            ("Post-enhancer PCA", out / "feature_post_enhancer_pca.png"),
            ("Pre feature norm", out / "feature_pre_norm.png"),
            ("Post feature norm", out / "feature_post_norm.png"),
        ])

    if "spatial" in items:
        save_spatial_bundle(out, rgb, snapshot["spatial_aux"])
        for name in ("spatial_gate_overlay.png", "spatial_delta_abs_overlay.png",
    ...
    if "view" in items:
        save_view_response(out, rgb, snapshot["stage1_end_points"], snapshot["bundle"])
        for name in ("view_entropy.png", "view_margin.png"):
            p = out / name
            if p.is_file():
                images.append((name, p))

    qids = snapshot["top_query_indices"].detach().cpu().tolist()
    if "cdf" in items:
        save_center_cdf_panels(
            out / "dcr_center_cdf.png", snapshot["cdf_logits"],
            snapshot["bundle"]["actions"][:, 0, 15] * 0 +
            snapshot["bundle"]["actions"].new_tensor(
                snapshot["bundle"]["actions"].shape[0] * [0.0])
            if False else snapshot["selected_offsets_mm"].new_tensor(
                snapshot["bundle"]["actions"].shape[0] * [0.0]),
            qids)
        # The generic helper needs the actual global offset grid; caller will
        # overwrite the placeholder panel below with the model grid.
        save_stage1_angle_depth(
            out / "stage1_angle_depth.png",
            snapshot["stage1_end_points"],
            snapshot["bundle"]["query_ids"], qids)
        images.append(("Stage-1 angle/depth response", out / "stage1_angle_depth.png"))

    if "query_response" in items:
        save_query_response(
            out, rgb, snapshot["bundle"]["token_ids"],
            snapshot["stage1_score"], snapshot["selected_offsets_mm"],
            snapshot["local_utility"])
        images.extend([
            ("Stage-1 query score", out / "query_stage1_score.png"),
            ("DCR selected offset", out / "query_selected_offset_mm.png"),
            ("Best local utility", out / "query_best_local_utility.png"),
        ])

    if "local" in items:
        p = out / "local_attention_overlay.png"
        save_local_patch_overlay(
            p, rgb, snapshot["local_debug"], max_queries=local_queries)
        images.append(("CVA local analysis / attention", p))
    ...
    if e1_model is not None:
        e1 = inspect_e1_like_case(
            e1_model, batch=None, bundle=snapshot["bundle"],
            active_depth=snapshot["active_depth"],
            depth_pack=snapshot["pack"])
        # The helper requires batch; patched by caller before use.
        method_arrays["e1"] = e1
    ...
    if air_model is not None and "air" in items:
        ...
        residual, diag = air_model.reader(
            snapshot["feature_pre"].detach(),
            snapshot["bundle_batch_K"],
            (h, w),
        ...
        method_arrays["air"] = air_action.cpu().numpy()
        air_info = {
            "residual": residual, "selected": air_sel,
            "selected_offsets_mm": air_off, "utility": air_u, "diag": diag,
        }

    if "grasps" in items:
        active_pts, active_cols = depth_to_points(
            snapshot["active_depth"], K, rgb, stride=point_stride)
        for method, arr in list(method_arrays.items()):
            if not isinstance(arr, np.ndarray):
                continue
            p = out / f"grasps_{method}_overlay.png"
            save_grasp_overlay(p, rgb, arr, K, f"{method} Top-{topk}", topk=topk)
            images.append((f"{method} grasps", p))
            ply = out / f"grasps_{method}_scene.ply"
            if save_grasp_scene_ply(
                    ply, active_pts, active_cols, arr, topk=topk):
                files.append((f"{method} grasp scene PLY", ply))

    return images, files, method_arrays, air_info
