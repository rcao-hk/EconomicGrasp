...
run_id=`20260926_stage1_rescue_repeat_seed0`，已完成：验证 MAE 16.135053 mm、
前景 MAE 25.296400 mm、局部斜率 0.358490。两次正常短段源状态及全部 100 步
输入/loader 完全一致，但终点 MAE 相差 2.588587 mm，说明同一配方的长轨迹有可观波动。
移除组的 4.859918 mm 优于两次正常短段，仍不能用单次移除轨迹代替跨 seed 证据，
也没有因此排除不同执行入口/诊断节奏对历史重放的影响。
