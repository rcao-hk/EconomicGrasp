...
Use the controlled Stage-1 RGB checkpoint and deterministic image-FPS queries:
...
`GPUS` is not consumed by the Python script; select the visible device externally,
for example:

```bash
CUDA_VISIBLE_DEVICES=0 SAMPLE_INTERVAL=0.1 bash run_center_decoupling_diag.sh
```
