...
WORK_ROOT=${WORK_ROOT:-/data2/robotarm/result/grasp/rgbgrasp/rep_c2v2_fullpath_verifier}
...
VAL_MOVE_LIMIT=${VAL_MOVE_LIMIT:-32}
  ...
  case "$phase" in
    ...
    train)
      args=(
        ...
        --train-cache-root "$WORK_ROOT/train_cache"
    ...
    test)
      ...
      for split in "${TESTS[@]}"; do
        launch "rep-c2v2/test/$split" "${GPU_IDS[$slot]}" "$WORK_ROOT/logs/test_${split}.log"           "$ROOT_DIR/test_rep_c2v2.py"           --source-root "$SOURCE_ROOT"           --cache-root "$CACHE_ROOT"           --c2v2-dir "$WORK_ROOT/train"           --output-root "$WORK_ROOT/eval"           --split "$split"           --cases "$CASES"           --max-files "$MAX_TEST_FILES"
        slot=$((slot+1))
