...
def safe_minmax(x, eps=1e-8):
    ...
    - normal case: (x - min) / (max - min)
    - constant positive vector: all ones
    - constant zero vector: all zeros
    """
...
def get_save_root(dataset_root, depth_type, mode):
    ...
    # 避免覆盖原始 scene graspness
    if mode == 'instance':
        base = base + '_instance'

    return os.path.join(dataset_root, base)


def generate_scene(scene_id, cfgs):
    dataset_root = cfgs.dataset_root
    virtual_dataset_root = cfgs.virtual_dataset_root
    camera_type = cfgs.camera_type
    save_path_root = get_save_root(dataset_root, cfgs.depth_type, cfgs.mode)
    ...
    for ann_id in range(256):
        print(f'generating mode={cfgs.mode} scene: {scene_id} ann: {ann_id}')
        ...
        if cfgs.depth_type == 'virtual':
            depth = np.array(Image.open(
                os.path.join(
                    ...
                    'scene_' + str(scene_id).zfill(4),
                    camera_type,
                    str(ann_id).zfill(4) + '_depth.png'
                )
            ))
            ...
            depth = np.array(Image.open(
                os.path.join(
                    ...
                    camera_type,
                    'depth',
                    str(ann_id).zfill(4) + '.png'
                )
            ))
        else:
            raise ValueError(f'Unsupported depth_type: {cfgs.depth_type}')
        ...
        for i, (obj_idx, trans_) in enumerate(zip(obj_list, pose_list)):
            ...
            valid_grasp_mask = (
                (fric_coefs <= fric_coef_thresh) &
                (fric_coefs > 0) &
                (~collision)
            )
            valid_grasp_mask = valid_grasp_mask.reshape(num_points, -1)

            # raw graspness: 与原脚本一致
            raw_graspness = np.sum(valid_grasp_mask, axis=1).astype(np.float32) / point_grasp_num

            # mode switch:
            # - scene: 保持原始 raw graspness
            # - instance: 先在每个 object / instance 内部做一次 min-max
            if cfgs.mode == 'scene':
                graspness = raw_graspness
            elif cfgs.mode == 'instance':
                graspness = safe_minmax(raw_graspness)
            else:
                raise ValueError(f'Unsupported mode: {cfgs.mode}')
            ...
            grasp_points.append(target_points.astype(np.float32))
            grasp_points_graspness.append(graspness.reshape(num_points, 1).astype(np.float32))

        grasp_points = np.vstack(grasp_points).astype(np.float32)
        grasp_points_graspness = np.vstack(grasp_points_graspness).astype(np.float32)

        grasp_points = torch.from_numpy(grasp_points).cuda()
        grasp_points_graspness = torch.from_numpy(grasp_points_graspness).cuda()

        cloud_masked_t = torch.from_numpy(cloud_masked.astype(np.float32)).cuda()
        grasp_points_t = grasp_points.contiguous().unsqueeze(0)
        cloud_masked_t = cloud_masked_t.contiguous().unsqueeze(0)

        _, nn_inds, _ = knn_points(cloud_masked_t, grasp_points_t, K=1)
        nn_inds = nn_inds.squeeze(-1).squeeze(0)

        cloud_masked_graspness = torch.index_select(
            grasp_points_graspness, 0, nn_inds
        ).cpu().numpy().astype(np.float32)

        # 最后统一做一次 scene-level normalization:
        # - scene mode: 这就是原始脚本行为
        # - instance mode: 对应论文里的第二步 Eq.(2)
        cloud_masked_graspness = safe_minmax(cloud_masked_graspness)
        ...
        np.save(
            os.path.join(save_path, str(ann_id).zfill(4) + '.npy'),
            cloud_masked_graspness_f16
        )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mode', default='instance', choices=['scene', 'instance'],
                        help='graspness normalization mode')
