...
parser.add_argument(
    ...
    action='store_true',
    help=(
        'Cache full per-view-per-angle-per-depth labels for a depth-conditioned '
        'width head and friction-conditioned success CDF head. Requires --extend_angle.'
    ),
)
parser.add_argument(
    '--cdf_thresholds',
    ...
    help=(
        'Comma-separated friction thresholds. cdf_bins=0 means failure at all '
        'thresholds; 1..T denotes the first threshold at which the grasp succeeds.'
    ),
)
parser.add_argument(
    '--max_grasp_width',
    type=float,
    default=0.1,
    help='Maximum valid gripper width in meters for CDF/width supervision.',
)
parser.add_argument(
    '--save_raw_friction',
    action='store_true',
    help=(
        'Also cache raw friction labels as float16 [Ns,K,A,D]. This is useful '
        'for debugging or changing thresholds later, but increases storage.'
    ),
)
parser.add_argument(
    '--compressed',
    action='store_true',
    help='Use np.savez_compressed. Saves disk space but is slower to generate/load.',
)
parser.add_argument(
    '--save_folder',
    default=None,
...
def _parse_thresholds(text: str) -> torch.Tensor:
    values = [float(v.strip()) for v in str(text).split(',') if v.strip()]
    if not values:
        raise ValueError('--cdf_thresholds must contain at least one value.')
    if any(not np.isfinite(v) or v <= 0 for v in values):
        raise ValueError(f'Invalid CDF thresholds: {values}')
    if any(values[i] >= values[i + 1] for i in range(len(values) - 1)):
        raise ValueError(f'CDF thresholds must be strictly increasing: {values}')
    return torch.tensor(values, dtype=torch.float32)


def _build_cdf_depth_labels(
    ...
    """Build storage-efficient CDF and depth-wise width labels.

    Inputs:
        raw_friction:     [Ns,V,A,D], original GraspNet friction/tolerance label.
                          Smaller positive values are better; <=0/non-finite is failure.
        scene_width:      [Ns,V,A,D], width in meters.
        scene_collisions: [Ns,V,A,D], scene-level analytic collision flag.
        thresholds:       [T], increasing friction thresholds.

    Outputs:
        cdf_bins:          [Ns,V,A,D], uint8.
                           0 = failure at every configured threshold.
                           1..T = first threshold index at which the grasp succeeds.
        width_valids:      [Ns,V,A,D], bool. Width loss mask aligned with candidates
                           that succeed at at least one configured threshold.

    The six-dimensional binary CDF target is reconstructed during training:
        target[..., t] = (cdf_bin > 0) & (t >= cdf_bin - 1)

    Collision, invalid friction, excessive width, and grasps whose minimum friction
    exceeds the largest configured threshold are encoded as cdf_bin=0.
    """
    if raw_friction.shape != scene_width.shape or raw_friction.shape != scene_collisions.shape:
        raise ValueError(
            'raw_friction, scene_width, and scene_collisions must share shape; '
            f'got {raw_friction.shape}, {scene_width.shape}, {scene_collisions.shape}'
        )

    thresholds = thresholds.to(device=raw_friction.device, dtype=raw_friction.dtype)
    finite = torch.isfinite(raw_friction) & torch.isfinite(scene_width)
    geometry_valid = (scene_width >= 0) & (scene_width <= float(max_grasp_width))
    evaluator_success = (
        finite
        & (raw_friction > 0)
        & (raw_friction <= thresholds[-1])
        & geometry_valid
        & (~scene_collisions)
    )

    # torch.bucketize(right=False): boundaries[i-1] < x <= boundaries[i].
    first_threshold = torch.bucketize(
        raw_friction.contiguous(),
        thresholds,
        right=False,
    ) + 1  # 1..T for values <= max threshold; T+1 above the range.

    cdf_bins = torch.where(
        evaluator_success,
        first_threshold,
        torch.zeros_like(first_threshold),
    ).to(torch.uint8)

    width_valids = evaluator_success
    return cdf_bins, width_valids


def _gather_top_views_4d(x: torch.Tensor, view_index: torch.Tensor) -> torch.Tensor:
    """Gather [Ns,V,A,D] along view dim with [Ns,K] -> [Ns,K,A,D]."""
    A, D = x.shape[-2:]
    idx = view_index.unsqueeze(-1).unsqueeze(-1).expand(-1, -1, A, D)
...
if __name__ == '__main__':
    ...
    cdf_thresholds = _parse_thresholds(cfgs.cdf_thresholds)

    if cfgs.cache_cdf_depth_labels and not cfgs.extend_angle:
        raise ValueError('--cache_cdf_depth_labels requires --extend_angle.')
    if cfgs.max_grasp_width <= 0:
        raise ValueError('--max_grasp_width must be positive.')

    if cfgs.save_folder is not None:
        save_folder_name = cfgs.save_folder
    elif cfgs.extend_angle and cfgs.cache_cdf_depth_labels:
        save_folder_name = (
            f'economic_grasp_label_{keeping_views_numbers}views_'
            f'extend_angle_cdf_depth'
        )
    ...
    for label_path in sorted(os.listdir(scenes_data_folders)):
        ...
        for i, obj_idx in enumerate(obj_idxs):
            ...
            width = torch.from_numpy(
                object_labels['offsets'][:, :, :, :, 2]
            ).float()
            raw_friction = torch.from_numpy(object_labels['scores']).float()
            scores = raw_friction.clone()
            collision = torch.from_numpy(scene_collision[f'arr_{i}']).bool()

            # Preserve raw friction before legacy EconomicGrasp normalization.
            # Collision grasps are invalid for legacy score learning; keep the
            # collision tensor and raw friction separately.
            scores[collision] = 0
            scores[scores < 0] = 0
            ...
            scene_raw_friction.append(raw_friction)
        ...
        scene_raw_friction = torch.cat(scene_raw_friction, dim=0)
        ...
        scene_raw_friction = scene_raw_friction[filter_mask]
        ...
        # Build full-depth labels before legacy score normalization/collapse.
        scene_cdf_bins = None
        scene_width_valids_depth = None
        if cfgs.cache_cdf_depth_labels:
            scene_cdf_bins, scene_width_valids_depth = _build_cdf_depth_labels(
                raw_friction=scene_raw_friction,
                scene_width=scene_width,
                scene_collisions=scene_collisions,
                thresholds=cdf_thresholds,
                max_grasp_width=cfgs.max_grasp_width,
            )
        ...
        label_mask = (scene_scores > 0) & (scene_width <= float(cfgs.max_grasp_width))
        ...
        if cfgs.extend_angle:
            ...
            if cfgs.cache_cdf_depth_labels:
                scene_cdf_bins = _gather_top_views_4d(scene_cdf_bins, index)
                scene_width_depth = _gather_top_views_4d(scene_width, index)
                scene_width_valids_depth = _gather_top_views_4d(
                    scene_width_valids_depth.to(torch.uint8),
                    index,
                ).bool()
                if cfgs.save_raw_friction:
                    scene_raw_friction = _gather_top_views_4d(
                        scene_raw_friction,
                        index,
                    )
        else:
        ...
        if cfgs.cache_cdf_depth_labels:
            # cdf_bins is compact: one ordinal byte instead of six binary bytes.
            save_dict['cdf_bins'] = scene_cdf_bins.numpy().astype(np.uint8)
            save_dict['cdf_thresholds'] = cdf_thresholds.numpy().astype(np.float32)
            save_dict['cdf_num_thresholds'] = np.array(
                [len(cdf_thresholds)],
                dtype=np.uint8,
            )
            save_dict['cdf_label_version'] = np.array([1], dtype=np.uint8)

            # Preserve one width label for every physical depth. Store millimeters
            # as uint16 to avoid float storage and overflow assumptions.
            widths_depth_mm = np.rint(
                np.clip(
                    scene_width_depth.numpy(),
                    0.0,
                    float(cfgs.max_grasp_width),
                ) * 1000.0
            ).astype(np.uint16)
            save_dict['widths_depth_mm'] = widths_depth_mm
            save_dict['width_valids_depth'] = (
                scene_width_valids_depth.numpy().astype(np.uint8)
            )
            save_dict['max_grasp_width'] = np.array(
                [cfgs.max_grasp_width],
                dtype=np.float32,
            )

            if cfgs.save_raw_friction:
                save_dict['friction_raw'] = (
                    scene_raw_friction.numpy().astype(np.float16)
                )

        save_path = os.path.join(
            save_data_folders,
            f'{label_path}_labels.npz',
        )
        if cfgs.compressed:
